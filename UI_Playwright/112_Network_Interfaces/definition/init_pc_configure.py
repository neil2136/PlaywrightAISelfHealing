...
class TestConfigPC2(Test):
    ...
    def test_01_initialize_pytest_bin_script(self):
        ...
        logger.info('Check "tests" in output ......')
        rc = 'tests' in out
        Assertion.assert_equal(rc, True, "ERR: Copy test script failed!")
    
    def test_02_copy_network_interface_script(self):
        logger.info('Copy test suite script files ......')
        tests_path = CaseParam.testbed_path + '/tests'
        PC2_LOGIN.send_command(f"cp -rf {SCRIPT_PATH}/112_Network_Interfaces/* {tests_path}")
        logger.info('Check copy result ......')
        out = PC2_LOGIN.send_command(f"ls {tests_path}")
        logger.info(f'out is: {out}')
        logger.info('Check "test_" in output ......')
        count = out.count("test_")
        Assertion.assert_equal(count, 1, "ERR: Copy network interface script failed!")
